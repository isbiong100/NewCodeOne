...
class Dytt:
    ...
    def parse_movie_list_data(self, content):
        """
        解析列表页数据
        需要保存电影的名称以及电影的二级地址
        :param content:
        ...
        for rt in result_list:
            ...
            # 发送二级请求 获取电影详情页面
            yield self.parse_movie_detail_data(film_detail_link, film_dict)

    def parse_movie_detail_data(self, link, film_dict):
        """
        解析电影详情页数据
        :param content:
        :param film_dict:
    ...
    def running(self):
        ...
        for index in range(2, 4):
            ...
            # 3. 解析列表页数据
            for x in self.parse_movie_list_data(content):
                self.save_data(x)
